...
export default function Formvalidators(e, data) {
  ...
  switch (name) {
    ...
    case "discount":
     ...
     case 'checkIn':
      ...
      if (data.checkOut && value >= data.checkOut) return 'Check-in must be before check-out';
      return '';
    case 'checkOut':
      if (!value) return 'Check-out date is required';
      if (!data.checkIn) return 'Select check-in first';
      if (value <= data.checkIn) return 'Check-out must be after check-in';
      return '';
